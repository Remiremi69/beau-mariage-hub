      ...
      {price && (
        <span
          style={{
            ...
            fontWeight: 400,
            fontSize: 13,
            color: price === "Inclus" ? "rgba(232,221,208,0.35)" : "#c9a96e",
            flexShrink: 0,
            marginLeft: 12,
          }}
        >
...
const PriceLineRow = ({ line }: { line: PriceLine }) => (
  ...
  >
    ...
    <span
      style={{
        fontFamily: "'Jost', sans-serif",
        fontWeight: line.isIncluded ? 300 : 500,
        fontSize: line.isIncluded ? 13 : 14,
...
const Step10_Recap = ({ state, onPrev }: Step10Props) => {
    ...
    try {
      ...
      const { error } = await client
        ...
        .insert({
          ...
          date_mariage: state.date,
          guests_estimate: state.guests,
          ceremonie_laique: state.ceremonieLaique,
          vin_dhonneur: state.vinDhonneur,
          repas_formule: state.repas,
          repas_entree: state.repasEntree,
          repas_plat: state.repasPlat,
          repas_dessert: state.repasDessert,
      ...
      console.error("Erreur envoi Supabase:", err, { state, contact });
      setIsSuccess(true);
    } finally {
      setIsLoading(false);
    }
  ...
  // Choices data
  const menuSubtext = [
    state.repasEntree ? menuNames[state.repasEntree] : null,
    state.repasPlat ? menuNames[state.repasPlat] : null,
    state.repasDessert ? menuNames[state.repasDessert] : null,
  ]
    .filter(Boolean)
    .join(" · ");

  const optionsList = (state.options || [])
    .map((id) => OPTION_LABELS[id] || id)
    .join(" · ");
      ...
      >
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <ChoiceLine
              category="Invités"
              value={`${state.guests} personnes (estimation)`}
            />
            <ChoiceLine
              category="Cérémonie"
              value={
                state.ceremonieLaique
                  ? "Laïque"
                  : "Mariage civil uniquement"
              }
              price={state.ceremonieLaique ? "+ 800 €" : "Inclus"}
            />
            <ChoiceLine
              category="Vin d'honneur"
              value={vinLabels[state.vinDhonneur] || state.vinDhonneur}
              price={
                state.vinDhonneur === "decouverte"
                  ? "Inclus"
                  : `≈ ${formatPrice(
                      (state.vinDhonneur === "prestige" ? 18 : 38) *
                        state.guests
                    )}`
              }
            />
            <ChoiceLine
              ...
              price={`≈ ${formatPrice(
                (state.repas === "essentiel"
                  ? 65
                  : state.repas === "gastronomique"
                  ? 90
                  : 130) * state.guests
              )}`}
            ...
            <ChoiceLine
              category="Décoration"
              value={decoLabels[state.deco] || state.deco}
              price={
                state.deco === "champetre"
                  ? "Inclus"
                  : state.deco === "boheme"
                  ? "+ 600 €"
                  : "+ 1 200 €"
            ...
            <ChoiceLine
              ...
              value={photoLabels[state.photographe] || "Non sélectionné"}
              price={
                state.photographe === "none"
                  ? undefined
                  : state.photographe === "reportage"
                  ? "+ 1 800 €"
                  : "+ 3 200 €"
              }
            />
            <ChoiceLine
              category="DJ"
              value={djLabels[state.dj] || "Non sélectionné"}
              price={
                state.dj === "none"
                  ? undefined
                  : state.dj === "standard"
              ...
              <ChoiceLine
                ...
                price={
                  breakdown.subtotalOptions > 0
                    ? `+ ${formatPrice(breakdown.subtotalOptions)}`
                    : undefined
                }
              />
            </div>
        ...
        >
          ...
          <div
            ...
            className="max-sm:!px-6 max-sm:!py-7"
          >
            {/* Price lines */}
            {breakdown.lines.map((line, i) => (
              <PriceLineRow key={i} line={line} />
            ))}
            ...
            >
              <div className="flex justify-between">
                ...
                >
                  {formatPrice(breakdown.subtotalFixe)}
                </span>
              </div>
              <div className="flex justify-between">
                <span
                ...
                >
                  ≈ {formatPrice(breakdown.subtotalRepas)}
                </span>
              </div>
              {breakdown.subtotalOptions > 0 && (
                <div className="flex justify-between">
            ...
            <div
              style={{
                borderTop: "1px solid rgba(201,169,110,0.40)",
                marginTop: 20,
                paddingTop: 24,
              }}
            ...
            >
              <div>
                ...
                >
                  ...
                  <br />
                  Le total définitif sera établi après
                  <br />
                  confirmation du nombre exact
                </p>
              </div>
          ...
          >
            Les montants marqués "estimation" seront ajustés à la date butoir
            fixée ensemble, environ 6 semaines avant le mariage. Seul le nombre
            définitif d'invités modifiera ces lignes.
          </p>
        </motion.div>
          ...
          <AnimatePresence mode="wait">
            {isSuccess ? (
              ...
              >
                ...
                >
                  Nous vous contacterons dans les 24 heures pour confirmer
                  votre date et répondre à toutes vos questions.
                </p>
                <p
                  style={{
                    fontFamily: "'Jost', sans-serif",
              ...
              >
                ...
                >
                  ...
                  <br />
                  Un conseiller Limen vous contacte sous 24h
                  <br />
                  pour valider votre date et répondre à vos questions.
                </p>
                ...
                >
                  ...
                  >
                    ...
                    au sujet de votre mariage. Aucune diffusion à des tiers.
                  </p>
