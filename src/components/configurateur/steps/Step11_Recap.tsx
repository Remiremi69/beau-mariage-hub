...
import { buildTimeline, type TimelineMoment } from "@/lib/build-timeline";
import { PdfEsquisse, type PdfEsquisseHandle } from "../PdfEsquisse";


const TimelineMomentRow = ({ moment, index }: { moment: TimelineMoment; index: number }) => (
  <motion.div
  ...
  >
    ...
    {/* Point lumineux centré sur la ligne (ligne à 80px du bord du conteneur, point à 12px à droite de l'horaire) */}
    <div
      style={{
        position: "relative",
        width: 24,
    ...
    {/* Contenu narratif */}
    <div style={{ marginLeft: 20, flex: 1, maxWidth: 480 }}>
      <h4
        style={{
...
const TimelineBlock = ({ state }: { state: ConfigurateurState }) => {
  const moments = useMemo(() => buildTimeline(state), [state]);
  ...
  return (
    <motion.div
      ...
      className="w-full"
      style={{ maxWidth: 760, marginBottom: 64 }}
    >
      {/* Header */}
      <div className="flex flex-col items-center text-center" style={{ marginBottom: 48 }}>
        <div className="flex items-center justify-center gap-4">
          <span style={{ width: 40, height: 1, background: "rgba(201,169,110,0.45)" }} />
          <span
            style={{
              fontFamily: "'Jost', sans-serif",
              fontWeight: 400,
        ...
        >
          Votre histoire, heure par heure.
        </p>
      </div>

      {/* Timeline */}
      <div className="relative" style={{ paddingLeft: 0 }}>
        {/* Ligne verticale (alignée sur le centre du point : 60px horaire + 12px (centre du wrapper 24px)) */}
        <div
          aria-hidden
          style={{
            position: "absolute",
        ...
        {moments.map((moment, i) => (
          <TimelineMomentRow key={`${moment.horaire}-${i}`} moment={moment} index={i} />
        ))}
      </div>
    </motion.div>
  );
...
/* ════════════════════════════════════════════════════════ */
const Step11_Recap = ({ state, onPrev, onUpdate }: Step10Props) => {
